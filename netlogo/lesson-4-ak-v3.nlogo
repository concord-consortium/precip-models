...
globals [
  is-NE?                                  ;true for NE map, false for AK
  ...
  presently-set-temp-unit
  temp-slider-max
  temp-slider-min

  N-incoming-temp       ;these variables replice variables from sliders for the simplified interface where changes
  N-incoming-moisture   ; to incoming temp and moisture can only be made for W and S
  E-incoming-temp
  E-incoming-moisture
]
...
;temp units from temperature-unit pull-down menu:
;  "Fahrenheit"
;  "Celsius"
...
to startup
  ca
  set is-NE? false
  set temperature-unit "Fahrenheit"  ;F is the default temp unit for Alaska
  set interpolation-method "weighted-average"
  set presently-set-temp-unit "Celsius"  ;this to trigger the conversion to Fahrenheit
  set temp-slider-min -20           ;initalize the min/max of the 4 sliders
  set temp-slider-max 30
  set N-incoming-temp 8             ;initialize the 4 temperature slider values
  set W-incoming-temp 8
  set E-incoming-temp 25
  set S-incoming-temp 8
  set N-incoming-moisture 4         ;initialize the 4 moisture slider values
  set W-incoming-moisture 4
  set E-incoming-moisture 10
  set S-incoming-moisture 4
  setup
end

to setup
;  ca      leave the temp and moisture settings so, only clear certain things
  clear-drawing
  clear-patches
  clear-turtles
  set timestep 0.5
...
to go
  ...
  set N-incoming-temp W-incoming-temp         ;since with this version, we don't see the setting of the
  set N-incoming-moisture W-incoming-temp     ; North incoming vars, we will keep them the same as the West
;  print (word "N-incoming-temp: " N-incoming-temp)
;  print (word "W-incoming-temp: " W-incoming-temp)
  ;update-interp  ;;not needed if only one interpolation-method, set as constant
...
to add-new-airs
  let W-in-temp to-C W-incoming-temp
  let E-in-temp to-C E-incoming-temp
  let N-in-temp to-C W-incoming-temp   ; N-incoming-temp
  let S-in-temp to-C S-incoming-temp
  ; West
  ask patches with [ pxcor = min-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-airs W-in-temp W-incoming-moisture -0.49 (0.5 - random-float 1)
  ]
  ; East
  ask patches with [ pxcor = max-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-airs E-in-temp E-incoming-moisture 0.49 (0.5 - random-float 1)
  ]
  ; North
  ask patches with [ pycor = max-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-airs W-in-temp W-incoming-moisture (0.5 - random-float 1) 0.49
  ]
  ; South
  ask patches with [ pycor = min-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-airs S-in-temp S-incoming-moisture (0.5 - random-float 1) -0.49
  ]
  ; NW
  ask patch min-pxcor max-pycor [
    let t W-in-temp ; (W-in-temp + N-in-temp) / 2
    let m W-incoming-moisture  ; (W-incoming-moisture + N-incoming-moisture) / 2
    let x -0.49
...
to update-visual
  ...
  set color (list (255 * (t / mt)) 0 (255 * (1 - t / mt)) 60) ;the last term, 125, makes about 50% transparency
...
to-report init-data
  ...
  [
    report [
      ...
      [42.8 -92 10 198 -0.6 -8.9] ;utica
      [42.3 -90.2 10 170 2.2 -8.9] ;albany
      [39.8 -92.2 10 170 -0.6 -8.9] ;allentown
...
to-report F-to-C [ temp-F ]
  report round ((temp-F - 32.0) * (5 / 9)) ;for display purposes in the slider, we round
end

to-report C-to-F [ temp-C ]
  report round ((temp-C * 1.8) + 32)
end

to-report to-C [ T ]
  report ifelse-value (temperature-unit = "Celsius") [ T ][ F-to-C T ]
end

to set-temp-slider-unit-change
  if presently-set-temp-unit != temperature-unit
  [
  ;print "temp unit changed"
  ifelse temperature-unit = "Celsius" ;this is the new unit
    [
      set temp-slider-min -20
      set temp-slider-max 30
      set N-incoming-temp F-to-C N-incoming-temp
      set E-incoming-temp F-to-C E-incoming-temp
      set S-incoming-temp F-to-C S-incoming-temp
      set W-incoming-temp F-to-C W-incoming-temp
    ]
    [
      set temp-slider-min -4
      set temp-slider-max 86
      set N-incoming-temp C-to-F N-incoming-temp
      set E-incoming-temp C-to-F E-incoming-temp
      set S-incoming-temp C-to-F S-incoming-temp
      set W-incoming-temp C-to-F W-incoming-temp
    ]
