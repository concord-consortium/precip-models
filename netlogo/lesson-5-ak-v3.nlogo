...
globals [
  ...
  city-wind-data
  data-times-list
  presently-set-time-of-measurement
  presently-set-hours-since-first-measurement
...
to startup
  ...
  set is-NE? false
  set timestep 0.5
  ;set temperature-unit "Celsius"    ;so Celsius is the actual default temp unit
  ;set presently-set-temp-unit temperature-unit
  set N-incoming-temp 0   ;these initialize the 4 temperature slider values
  set W-incoming-temp 0
  set E-incoming-temp 50
  set S-incoming-temp 50
  set N-incoming-moisture 3  ;these initialize the 4 moisture slider values
...
to setup-stations-from-data [ data ]
  foreach data [ row ->
    create-stations 1 [
      ...
      set city item 6 row
      let city-data city-wind-values city
      ;print (word "City: " city " city-data: " city-data)
      let wind-speed item 2 row
      let wind-dir item time-index city-data
...
;reports the list that contains the name of the city that holds the wind direction for the 12 timed measurement
to-report city-wind-values [ cty ]
  foreach timed-wind-data [ x -> if (first x = cty) [report x]]
end
...
to go
  ...
  ifelse trails? [
    ask parcels [ if trail-maker? [pd] ]
  ] [
    if trails? != last-trails? [
      ask parcels [ pu ]
      if is-parcel? subject [ ask subject [ pd ] ]
      reset-drawing
    ]
  ]
...
;to read-timed-wind-data
;  file-open "ak-city-wind-data2.tsv.txt"
;
;  set data-times-list (sentence file-read-line)
;  let city-wind-list []
;  while [not file-at-end?][
;    let city-wind-list (sentence file-read-line)
;    ;print city-wind-list
;    set city-wind-data lput city-wind-list city-wind-data
;  ]
;  file-close
;end
...
to edit-stations
  ifelse mouse-down? [
;    if is-station? subject [   ;commented out code here to lock down stations
;      ask subject [
    ...
    if is-vector-head? subject [
      ask subject [
        ask link-neighbors [
          ...
          set station-wind-speed round station-wind-speed
          if station-wind-speed > 0 [
            set station-wind-direction (towards one-of link-neighbors) + (180 mod 360)
            set station-wind-direction round station-wind-direction
          ]
        ]
      ]
    ]

    if subject = nobody or is-parcel? subject [
...
;the two options for interpolation were:
;  "nearest-neighbor"
;  "weighted-average"
;but we are just going with weighted-average for now
to update-interp
...
to update-time-of-measurement
  if presently-set-hours-since-first-measurement != hours-since-first-measurement [
    set time-of-measurement item ((hours-since-first-measurement / 2) + 1) data-times-list
    set presently-set-hours-since-first-measurement hours-since-first-measurement
    set presently-set-time-of-measurement time-of-measurement
    setup
  ]
  if presently-set-time-of-measurement != time-of-measurement [
    let p time-index
    set hours-since-first-measurement ((p - 1) * 2)
    set presently-set-hours-since-first-measurement hours-since-first-measurement
    set presently-set-time-of-measurement time-of-measurement
    setup
  ]
...
to add-new-parcels
  ...
  ask patches with [ pxcor = min-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-parcels W-incoming-temp W-incoming-moisture -0.49 (0.5 - random-float 1)
  ]
  ; East
  ask patches with [ pxcor = max-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-parcels E-incoming-temp E-incoming-moisture 0.49 (0.5 - random-float 1)
  ]
  ; North
  ask patches with [ pycor = max-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-parcels N-incoming-temp N-incoming-moisture (0.5 - random-float 1) 0.49
  ]
  ; South
  ask patches with [ pycor = min-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-parcels S-incoming-temp S-incoming-moisture (0.5 - random-float 1) -0.49
  ]
  ; NW
  ...
  ask parcels with [ last-density = 0 ] [ set last-density density ]
end

to make-parcels [ incoming-temp incoming-moisture x-mod y-mod ]
  sprout-parcels random-poisson (timestep * new-parcel-rate)  [
    ...
    set trail-maker? ( random 5 = 0 )
    set birth-tick ticks
    update-visual
  ]
end

to-report new-parcel-rate
  report interpolate stations [-> sqrt (wind-x * wind-x + wind-y * wind-y)] / 5
end

to follow-parcel
  if mouse-down? and mouse-inside? [
    let new-subject min-one-of parcels [ distancexy mouse-xcor mouse-ycor ]
...
to-report density
  ;let nearby parcels at-points nearby-points ;(turtle-set parcels-here parcels-on neighbors)
  ;let r 2
  ;report sum [ 1 - (distance myself) / r  ] of other nearby
;  let r 2.9
;  let nearby (turtle-set other parcels-here parcels-on neighbors)
;  report sum [ 1 - (distance myself) / r ] of other nearby
  report sum [ count parcels-here ] of neighbors / 2 + count parcels-here
end
...
to update-visual
  ...
  set color (list (255 * (t / mt)) 0 (255 * (1 - t / mt)) 60)
  let m vapor-pressure
  if m < 0 [ set m 0 ]
  set size 0.1 + sqrt (m / 20)
  set hidden? not wind-visible?
end
...
to-report init-data
  ...
  [
    report [
      [43.5 -97.1 10 315 -3 -9] ;toronto
      [42.6 -96.3 10 329 -5.6 -13.3] ;buffalo
      [42.9 -94.9 10 321 5.6 -2.8] ;rochester
      [42.8 -93.1 10 293 -7.8 -12.2] ;syracuse
      [42.8 -92 10 235 -0.6 -8.9] ;utica
      [42.3 -90.2 10 170 2.2 -8.9] ;albany
      [39.8 -92.2 10 185 -0.6 -8.9] ;allentown
      [40 -90.6 10 196 -0.6 -8.9] ;new york city
      [42.9 -87.6 10 211 -0.6 -8.9] ;concord
      [43.5 -86 10 183 -0.6 -8.9] ;portland
                                  ;[40.8 -91.3 10 180 -0.6 -8.9] ;fudge
    ]
  ]
  [
    report [
      ...
      [40.4 -94.3 10 290 -0.6 -8.9 "St. Mary's" ]
...
to-report timed-wind-data
  report [
    ...
    ["Noorvik"	188	190	228	270	300	305	315	310	310	315	315	320	320 285]
    ["Nome"	203	203	231	286	305	312	318	323	321	320	316	324	325 300]
    ...
    ["Galena"	170	197	177	167	170	168	180	210	245	240	242	240	243 304]
    ["St. Mary's"	170	197	196	167	170	168	180	190	178	200	180	190	221 263]
    ["Cape Newenham"	215	215	217	205	190	185	179	164	190	200	200	179	190 246]
  ]
end
...
to update-measurements
  let nearby parcels in-radius 3
  ifelse any? nearby [
...
to rain
  ...
  ; convergence > 0: parcels are coming together
  let temp-at-1km temp - 8
  if vapor-pressure > sat-vapor-pressure temp-at-1km and convergence > 0.01 and temp > 16 and not on-border? [
