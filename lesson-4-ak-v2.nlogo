...
to startup
  ca
  set is-NE? false
  set temperature-unit "Celsius"    ;Celsius is the  default temp unit
  ...
  set N-incoming-temp 2             ;initialize the 4 temperature slider values
  set W-incoming-temp 2
  set E-incoming-temp 14
  set S-incoming-temp 14
  set N-incoming-moisture 4         ;initialize the 4 moisture slider values
  set W-incoming-moisture 3
  set E-incoming-moisture 4
  set S-incoming-moisture 8
  setup
end
...
to add-new-airs
  ...
  ask patches with [ pxcor = min-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-airs to-C W-incoming-temp W-incoming-moisture -0.49 (0.5 - random-float 1)
  ]
  ; East
  ask patches with [ pxcor = max-pxcor and pycor != max-pycor and pycor != min-pycor ] [
    make-airs to-C E-incoming-temp E-incoming-moisture 0.49 (0.5 - random-float 1)
  ]
  ; North
  ask patches with [ pycor = max-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-airs to-C N-incoming-temp N-incoming-moisture (0.5 - random-float 1) 0.49
  ]
  ; South
  ask patches with [ pycor = min-pycor and pxcor != max-pxcor and pxcor != min-pxcor ] [
    make-airs to-C S-incoming-temp S-incoming-moisture (0.5 - random-float 1) -0.49
  ]
  ; NW
  ask patch min-pxcor max-pycor [
    let t (to-C W-incoming-temp + to-C N-incoming-temp) / 2
  ...
  ask patch max-pxcor max-pycor [
    let t (to-C E-incoming-temp + to-C N-incoming-temp) / 2
  ...
  ask patch max-pxcor min-pycor [
    let t (to-C E-incoming-temp + to-C S-incoming-temp) / 2
  ...
  ask patch min-pxcor min-pycor [
    let t (to-C W-incoming-temp + to-C S-incoming-temp) / 2
